...
```
When document lengths were short, position2 and postion4 were least distant than all other pairs (3.00). Now, those positions are more distant (5.10) than two other pairs.
...
```
Likewise, position2 and position4 were more similar than all other pairs (0.309). Like Euclidean distance, the value has changed, but not as much. Now, those positions (0.227) are less similar than one other pair.
...
# Cosine similarity 
news_source_issue_cos_sim = as.dist(textstat_simil(news_dfm_sources_issue, method = "cosine", margin="documents")) 
...
p1 = ggplot(all_pairs_tob, aes(x=similarity)) +
    geom_density() +
    ggtitle("Similarity Across Tobacco Article Sources") +
    xlim(0,1)

p2 = ggplot(all_pairs_ss, aes(x=similarity)) +
    geom_density() +
    ggtitle("Similarity Across Same-Sex Article Sources") +
    xlim(0,1)

grid.arrange(p1, p2, ncol=2)

t.test(all_pairs_ss$similarity, all_pairs_tob$similarity)
